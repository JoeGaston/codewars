...
function giveMeSunshine(dailyTemps) {
    ...
    for (let i = 0; i <= dailyTemps.length; i++) {
        ...
        } else if (dailyTemps[i] > dailyTemps[i + 1]) { // we need to stop and enter into another conditional here that can begin to count 
            //number of days until it finds a greater number. After that, the counter would be used to assign the value to the 
            //daysUntilWarm array and then count down to 0?
            //counter +=1
            //console.log(counter)
